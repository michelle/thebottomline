...
describe EcardMailer do
  ...
  describe 'ecard confirmation sending' do
    let(:ecard) { mock_model(Ecard, :sender => 'Eric',
                                  :recipient => 'Granny', 
                                  :address => 'koolgranny@email.com',
                                  :message => 'test!') }
    let(:mail) { EcardMailer.ecard_confirmation_email('testemail@email.com', ecard) }
 
    #ensure that the subject is correct
    it 'renders the subject' do
      mail.subject.should == "Your ecard has been sent"
    end
 
    #ensure that the receiver is correct
    it 'renders the receiver email' do
      mail.to.should == ['testemail@email.com']
    end
